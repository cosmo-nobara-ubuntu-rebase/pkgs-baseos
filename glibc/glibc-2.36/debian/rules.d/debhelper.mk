...
ifeq (amd64,$(DEB_HOST_ARCH))
	# libc6 ld-linux-x86-64.so.2 symlink fix
	rm -f $(curpass)/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2
	ln -sf ld-2.36.so $(curpass)/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2
	chmod +x $(curpass)/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2
	chmod +x $(curpass)/lib/x86_64-linux-gnu/ld-2.36.so
endif
endif
ifeq (libc6,$(curpass))
	dh_link -p$(curpass)
ifeq (i386,$(DEB_HOST_ARCH))
	# libc6 ld-linux.so.2 symlink fix
	rm -f $(curpass)/lib/ld-linux.so.2
	rm -f $(curpass)/lib/i386-linux-gnu/ld-linux.so.2
	ln -sf ld-2.36.so $(curpass)/lib/ld-linux.so.2
	ln -sf ld-2.36.so $(curpass)/lib/i386-linux-gnu/ld-linux.so.2
	chmod +x $(curpass)/lib/ld-linux.so.2
	chmod +x $(curpass)/lib/i386-linux-gnu/ld-linux.so.2
	chmod +x $(curpass)/lib/i386-linux-gnu/ld-2.36.so
endif
endif
ifeq (libc6-x32,$(curpass))
	# libc6-x32 bin/ld.so symlink fix
	dh_link -p$(curpass)
	rm -f $(curpass)/libx32/ld-linux-x32.so.2 || echo
	ln -sf ld-2.36.so $(curpass)/libx32/ld-linux-x32.so.2|| echo
	chmod +x $(curpass)/libx32/ld-2.36.so || echo
	chmod +x $(curpass)/libx32/ld-linux-x32.so.2  || echo
endif
# Do not Override the rest
ifneq (libc6,$(curpass))
	dh_link -p$(curpass)
else ifneq (libc6-x32,$(curpass))
	dh_link -p$(curpass)
endif
